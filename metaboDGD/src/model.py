...
class MetaboDGD():
    ...
    def __init__ (
        ...
        self.dec = Decoder(
            ...
            abundance_gaussian_stddev=abundance_gaussian_stddev,
        )
    ...
    def get_representations(
        ...
        data_loader = DataLoader(
            dataset=metabolite_ds,
            batch_size=opt_config['batch_size'],
            shuffle=False
        )

        ## Assume for now one representation per component initialized per sample
        rep_init = self.gmm.sample_new_points(len(metabolite_ds))
        ...
        optimizer = Adam(
            ...
            lr=opt_config['opt']['options']['lr'], # From lr_rep in training
            weight_decay=opt_config['opt']['options']['weight_decay'],
            betas=opt_config['opt']['options']['betas']
        )
        ...
        for e in range(0, opt_config['opt']['epochs']):
            optimizer.zero_grad()
            for x, i in data_loader:
                n_batch_size = len(i)
